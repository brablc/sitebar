...
class Translator extends SB_ErrorHandler
{
    ...
    public $fmt = './locale/%s/%s';
    public $infofmt = './locale/%s/%s';
    public $langs = array();
    public $gid = null;
    public $plugin = '';
    public $pluginCGI = '';
    ...
    public function addFile(&$zipfile, $lang, $file)
    {
    ...
    public function check($needAdmin = false)
    {
...
if ($missing) :
    ?><br>[<a href='translator.php?lang=<?php echo $lang?>&amp;<?php echo $this->pluginCGI ?>cmd=add&amp;edit=<?php echo $part?>'>ADD</a>]<?php
endif;
if ($update && !$this->parts[$part]['inline']) :
    ?><br>[<a href='translator.php?lang=<?php echo $lang?>&amp;<?php echo $this->pluginCGI ?>cmd=upd&amp;edit=<?php echo $part?>'>UPD</a>]<?php
endif;
    }
                }
            }
    ...
    public function encodeValue($value, $show = false)
    {
        if ($show) {
    ...
    public function decodeValue(&$param, $value)
    {
        $value = str_replace("&#39;", "'", $value);
        return stripslashes($value);
    }

    public function edit($lang, $part, $cmd, $save)
    {
        ...
        if ($save) {
            ...
            foreach ($default as $label => $value) {
                ...
                if (isset($data[$label]) && $cmd == 'add') {
                    $value = $data[$label];
                } elseif (isset($values[$label]) && ($changed || isset($data[$label]))) {
                    $value = $changed ? html_entity_decode($values[$label]) : $data[$label];
                } elseif (!$inline && $cmd == 'upd' && !$changed && isset($data[$label])) {
                    $value = $data[$label];
                } else {
                    $value = '';
                }
