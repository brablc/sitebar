...
class SB_Writer_sitebar extends SB_WriterInterface
{
    ...
    public $sortMode;
    public $useHitCounter = false;
    public $useToolTips = false;
    public $showACL;

    public $linkCount = 0;
    public $nodeCount = 0;
    public $loadOpenNodesOnly = true;

    public $appError = null;
    ...
    public function setOutputHandler()
    {
        ...
        if (
            $this->um->getParam('config', 'use_compression')
    ...
    public function getExpandedNodes($cookieName)
    {
        $states = null;
        $nodes = array();
    ...
    public function writeMenu($type, $items)
    {
        ?>
    ...
    public function drawJavaScriptData()
    {
...
';
            ...
            if ($interval > 0 && !$this->hasErrors()) {
                ...
                if (!$this->um->db->getData('version', 'failed_count')) {
                    ...
                    if ($lastVersion && version_compare(SB_CURRENT_RELEASE, $lastVersion, '<')) {
                        ...
                        echo '<div id="versionCheck">';
                        $link = sprintf(
                            ' <a href="%s"' . SB_Page::target() . '>%s</a>',
                            $this->um->db->getData('version', 'Download URL'),
                            $lastVersion
                        );
    ...
    public function drawFoot()
    {
        ...
        if (
            $this->um->getParam('config', 'show_statistics')
