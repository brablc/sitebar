...
Opera kullanıcıları ise SiteBar "Kullanıcı Ayarları" kısmında bulunan "Menu Simgesini Göster" seçeneğini etkinleştirmelidirler. Bu özelliği etkinleştirdikten sonra klasör veya baglantının yanında çıkan simgeye sol tuşla tıklayarak menüyü açabilirler. Ayrıca Klasör veya Linkin yanındaki yazıya Ctrl tuşuna basılı iken farenin sol tuşuyla tıklanarak da aynı şekilde menü açılabilir.
_P;
...
Soldaki URL(internet adresi) <IFRAME&gt içinde SiteBar oluşturmayı sağlar;. Bu adres <ul> <li><a href="http://www.pageflakes.com/">Pageflakes</a> <li><a href="http://www.netvibes.com/">Netvibes</a> </ul> gibi çeşitli portallara SiteBar'ı eklemeyi sağlar. Portalı ziyaret edin ve içerik ekleme kısmını bulun. Bu URL'yi ( <strong>%s</strong> ) oraya kopyalayın böylece yeni bir bölüm oluşturulacaktır. (Genellikle bu portallar https adreslerini desteklemez , bunun için iframe.php kullanabilirsiniz). Kullanıcı adı ve parola bu portallara <strong>aktarılmaz</strong>. Bunun yerine Internet Explorer kullanıcıları çerezlere (cookies) izin vererek SiteBar'da şifreleri hatırlatabilir.
_P;

$para['integrator::hint_google'] = <<<_P
Bu da IFRAME kullanır fakat Google'ın Kişiselleştirilmiş Anasayfa'sında gösterilmek üzere ayarlanmıştır. 
<strong>Nesne Ekle</strong> ve bu adresi kullanın: <strong>%s</strong> .
_P;
...
&#42; <i><a href="http://en.wikipedia.org/wiki/Bookmarklet">Bookmarklet</a> içinde JavaScript kodu bulunduran bir bağlantıdır. Buna sağ tuşla tıklayarak bağlantılar(bookmarks/favorites) klasörüne ekleyebilirsiniz. Daha sonra bu bağlantıya tıklanarak içindeki JavaScript kodu çalıştırabilir. </i>
_P;
...
Eklenti SiteBar' özel oluşturulmuştur. Bu eklenti bir klasördeki tüm bağlantıları sekmelerde açmayı sağlar ve başka özellikler de sunar. SiteBar simgelerini Araç çubuğuna eklemek için Görünüm/Araç Çubuğu/Kişiselleştir(View/Toolbars/Customize) menüsünü kullanınız.
_P;

$para['integrator::hint_bmsync'] = <<<_P
Firefox'da iki taraflı bağlantı eşitlemesi yapmak için lütfen "Bookmark Synchronizer" eklentisini indirin. Eşitlemenin nasıl kurulacağını öğrenmek ve daha fazla bilgi almak için "Kullanıcı Ayarları -> XBELSync Ayarları" kısmına bakınız. [<a href="http://sitebar.org/downloads.php">Daha Fazla Bilgi</a>]
_P;
...
Şu aşamaları takip edin: <ol> <li>Konqueror Programını açınız <li> "Window -> Show Navigation Panel (F9)" menüsüne giriniz <li>En soldaki simgelerin(icon) bulunduğu barda bulunan simgelerin altındaki Navigation Panel'e sağ tuşla tıklayın. <li>"Add New -> Web SideBar Module" seçeneğini seçin -  "Web SiteBar Plugin" adında bir simge eklenecektir. <li> Bu yeni simgeye sağ tuşla tıklayın ve "Set Name" seçeneğini seçin. Buraya <b>SiteBar</b> yazın. <li> Bu simgeye sağ tuşla tıklayın ve "Set Url"'yi seçerek, <b>%s</b> yazın. <li>Simgeye tıklayarak SiteBar'ı SideBar'da açabilirsiniz. </ol>
_P;

$para['integrator::hint_hotlist'] = <<<_P
Opera linklerinize SiteBar linki eklenecektir. Linklerinizin listesindeki bu linke tıklayarak Opera Panellerine SiteBar'ı ekleyebilirsiniz.
_P;
...
%s
<p>
Klasör veya linke sağ tuşla tıklayarak linklerinizi düzenleyebilirsiniz. 
<p>
Veya "%s" seçeneğini "%s" bölümünden aktif hale getirdikten sonra çıkan menü simgesini de kullanabilirsiniz.
<p>
...
Yönetici hesap açma isteğinizi kabul etti. %s kullanıcı adını kullanarak kullanarak giriş yapabilirsiniz.
--
...
Yönetici hesap açma isteğinizi reddetti (kullanıcı adı : %s)
...
Yönetici %s kullanıcı adı ile kullandığınız hesap aktif olmadığı için, hesabınızı sildi.
...
<p> Bir grubu terk ettiğinizde tekrar üye olmak için davete ihtiyacınız vardır. Daveti alabilmek için grup sahibi ile temasa geçmeniz gerekmektedir. Bunun için bu kişinin SiteBar kullanıcı adını veya email adresini bilmeniz gerekir.
_P;

$para['command::use_comma'] = <<<_P
Yeni bir satıra geçerek veya virgül kullanarak kullanıcı adlarını / email adreslerini ayırın. Davetinizi kabul eden kullanıcılar üye olacaklardır.
_P;

$para['command::reset_password_hint'] = <<<_P
<p>
Lütfen kullanıcı adı veya kayıtlı e-mail adresi kısmını doldurunuz. Kayıtlı e-mail adresinize bir jeton(link) gönderilecektir. Gelen bu linki şifrenizi sıfırlamak için kullanabilirsiniz.
_P;
...
<h3>Hesabınızı gerçekten silmek istiyor musunuz?</h3> Bu işlemin geriye dönüşü yoktur!
_P;

$para['command::email_link_href'] = <<<_P
<p>Kendi E-mail programınızla <a href="mailto:?subject=Web sitesi %s&body=İlgilenebileceğini düşündüğüm bir site buldum. Bir bakmanı tavsiye ederim : %s
--
%s adresindeki SiteBar Bağlantı(Bookmark) Yöneticisi tarafından gönderilmiştir. SiteBar Hakkında daha çok şey öğrenmek için tıklayın: http://sitebar.org">e-mail gönderin</a>
...
SiteBar'in e-mail özelliklerini kullanmanızı sağlayan e-mail doğrulamasını istediğiniz için teşekkür ederiz. 

Lütfen aşağıdaki bağlantıya tıklayarak e-mail adresinizi doğrulayınız:

   %s

Bu e-mail bilginiz dışında gelmişse dikkate almayınız.
...
Kurallar: <strong>O</strong>ku, <strong>E</strong>kle, <strong>D</strong>egiştir, <strong>S</strong>il
_P;
...
Grubu olmayan kullanıcılar için oluşturulacak grupların listesi. | işaretini kullanarak grup adlarını ayırın.
_P;

$para['command::tooltip_public_groups'] = <<<_P
Anonim kullanıcıların erişimine açık olan grupların listesi.
_P;
...
Bağlantı açıklamalarının en fazla ne kadar uzun olacağı belirlenebilir. Küçük dosyaları açıklama olarak saklamak mümkündür.
_P;
...
Gelişmiş kontrolleri ve daha fazla bilgi mesajı göster.
_P;
...
Grupları listeden seçmek yerine filtreler kullanın.
_P;

$para['command::tooltip_filter_users'] = <<<_P
Üyeleri listeden seçmek yerine filtreler kullanın.
_P;
...
SiteBar HTTPS adreslerini doğrulayamaz. Eğer işaretlenmezse HTTP adresi olmayan adresler doğrulamayı geçmemiş sayılır.
_P;
...
Varsayılan ayar olarak SiteBar my.sitebar.org adresindeki bütünleyiciyi kullanır, bu ayar sitenin kendi bütünleyicisini kullanacak şekilde değiştirilebilir(güvenlik sebebiyle).
_P;
...
Bağlantıları feed olarak işaretle - eğer ayarlandıysa bağlantı tarayıcı yerine feed okuyucu ile açılacaktır.
_P;

$para['command::tooltip_load_all_nodes'] = <<<_P
Tüm klasörleri yükle, az sayıda linki olup filtreleme özelliğini kullanan kullanıcılar için uygundur.
_P;

$para['command::tooltip_popup_params'] = <<<_P
Sitebar için açılan Pop-up penceresinin değişkenleri. Varsayılan değerleri geri almak için boş bırakın.
_P;
...
Yönetici maksimum oturum süresini belirleyebilir. Bu süre geçildiğinde, kullanıcının tekrar giriş yapması gerekir.
_P;
...
Özel bağlantının bulunduğu klasör yayınlansa bile sadece ağacın sahibi bu bağlantıyı görebilir.
_P;

$para['command::tooltip_private_over_ssl_only'] = <<<_P
SiteBar, sadece güvenli bağlantı(SSL) üzerinden kullanılıyorsa özel bağlantılar yüklenecektir.
_P;
...
SiteBar araç çubuğunda kullanılacak aramada kullanılacak simge(icon).
_P;

$para['command::tooltip_search_engine_url'] = <<<_P
Arama Motorunun URL'si. Arama ifadesinin kullanılacağı yerlerde %SEARCH% kullanın.
_P;

$para['command::tooltip_sender_email'] = <<<_P
Sitebar tarafından oluşturulan e-mailler bu adres ile gönderilecektir.
_P;
...
Tüm linkleri yeni bir pencerede _blank hedefi ile açar.
_P;
...
SiteBar yeni sürümünün mevcut olup olmadığını düzenli kontrol edebilir. Şu anki sürümde açıklar tespit edilebileceği için bu önemlidir. Dışarıya bağlantı gerektirir.
_P;

$para['command::tooltip_web_search_user_agents'] = <<<_P
Kullanıcı Araçları için (javascript'e dayanmayan writer alacak) bir düzgün ifade.
_P;
...
Kullanıcı adınız ve yanındaki simge sizin ana link klasörünüzdür. Bunlara sağ tuşla tıklayıp %s komutunu seçerek ilk linkinizi ekleyebilirsiniz. 
_P;

$para['sitebar::invitation'] = <<<_P
<strong>%s</strong> adlı kullanıcı linklerini sizinle paylaşmak istiyor ve sizi <strong>%s</strong> grubuna üye olmaya davet ediyor.
_P;

$para['usermanager::signup_info'] = <<<_P
"%s" adlı kullanıcı, %s adresindeki SiteBar kurulumunuza üye oldu.
_P;

$para['usermanager::signup_info_verified'] = <<<_P
"%s" adlı kullanıcı, %s adresindeki SiteBar kurulumunuza üye oldu. Kullanici e-mail adresini onayladi.
_P;
