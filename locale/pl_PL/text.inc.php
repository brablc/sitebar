<?php

$text['The Bookmark Server for Personal and Team Use'] = <<<_P
Serwer zakładek do użytku gupowego i osobistego
_P;

$text['SiteBar Homepage'] = <<<_P
Strona domowa SiteBar
_P;

$text['Your Language'] = <<<_P
Polski
_P;

$text['Charset in MS Windows'] = <<<_P
cp1250
_P;

$text['Add Link to SiteBar'] = <<<_P
Dodaj link do SiteBar
_P;

$text['Add Page to SiteBar'] = <<<_P
Dodaj stronę do SiteBar
_P;

$text['SiteBar Integrator'] = <<<_P
SiteBar Integrator
_P;

$text['Bookmarklet'] = <<<_P
Bookmarklet
_P;

$text['Bookmarks'] = <<<_P
Zakładki
_P;

$text['Homepage'] = <<<_P
Strona domowa
_P;

$text['Integration Instructions'] = <<<_P
Instrukcje integracji
_P;

$text['Linux Distro/Other Tools'] = <<<_P
Dystrybucja Linuxa/Inne narzędzia
_P;

$text['Mozilla Firefox'] = <<<_P
Mozilla Firefox
_P;

$text['Mozilla'] = <<<_P
Mozilla
_P;

$text['Microsoft Internet Explorer'] = <<<_P
Microsoft Internet Explorer
_P;

$text['Maxthon Tabbed Browser'] = <<<_P
Maxthon Tabbed Browser
_P;

$text['Opera Web Browser'] = <<<_P
Opera Web Browser
_P;

$text['Usage Tips for All Browsers'] = <<<_P
Porady użytkowania dla wszystkich przeglądarek
_P;

$text['Usage/Integration Tips for %s'] = <<<_P
Porady użytkowania/integracji dla %s
_P;

$text['Browser/Category'] = <<<_P
Przeglądarka/Kategoria
_P;

$text['Version/Platform'] = <<<_P
Wersja/Platforma
_P;

$text['Add Search Engine'] = <<<_P
Dodaj wyszukiwarkę
_P;

$text['Add to Panel'] = <<<_P
Dodaj do panelu
_P;

$text['Bookmarks Synchronizer Extension'] = <<<_P
Rozszerzenie Synchronizator Zakładek
_P;

$text['Debian'] = <<<_P
Debian
_P;

$text['Gentoo Ebuild'] = <<<_P
Gentoo Ebuild
_P;

$text['Install'] = <<<_P
Instalacja
_P;

$text['Live Bookmarks'] = <<<_P
Zakładki Live
_P;

$text['MozLinker Extension'] = <<<_P
Rozszerzenie MozLinker
_P;

$text['PHP Layers Menu'] = <<<_P
PHP Layers Menu
_P;

$text['Show in Search Bar'] = <<<_P
Pokaż w pasku wyszukiwania
_P;

$text['Sidebar'] = <<<_P
Sidebar
_P;

$text['Sidebar Plugin'] = <<<_P
Sidebar Plugin
_P;

$text['SiteBar Directory'] = <<<_P
Katalog SiteBar
_P;

$text['SiteBar Pop-up'] = <<<_P
SiteBar Pop-up
_P;

$text['SiteBar Sidebar Extension'] = <<<_P
Rozszerzenie SiteBar Sidebar
_P;

$text['Toolbar Plugin'] = <<<_P
Plugin paska narzędzi
_P;

$text['Uninstall'] = <<<_P
Deinstalacja
_P;

$text['Error'] = <<<_P
Błąd
_P;

$text['Warning'] = <<<_P
Uwaga
_P;

$text['Unknown'] = <<<_P
Nieznany
_P;

$text['Backend Bookmark Search'] = <<<_P
Wyszukiwanie zakładek w tle
_P;

$text['Filter Loaded Bookmarks'] = <<<_P
Filtruj załadowane zakładki
_P;

$text['Search Web'] = <<<_P
Szukaj w Sieci
_P;

$text['Collapse/Expand All'] = <<<_P
Ukryj/Pokaż wszytkie
_P;

$text['Reload'] = <<<_P
Odśwież
_P;

$text['Reload with Hidden Folders'] = <<<_P
Odśwież pokazując ukryte katalogi
_P;

$text['SiteBar Search Results'] = <<<_P
Wyniki wyszukiwania SiteBar
_P;

$text['%s Bookmark News'] = <<<_P
%s Najnowsze linki
_P;

$text['Continue with Web Search ...'] = <<<_P
Kontynuacja wyszukiwania w Sieci ...
_P;

$text['Web Search Results'] = <<<_P
Rezultaty wyszukiwania
_P;

$text['Commander'] = <<<_P
Okno poleceń
_P;

$text['Accept Membership'] = <<<_P
Akceptuj członkowstwo
_P;

$text['Accept'] = <<<_P
Akceptuj
_P;

$text['Active Users'] = <<<_P
Aktywni użytkownicy
_P;

$text['Add Bookmark'] = <<<_P
Dodaj link
_P;

$text['Add Folder'] = <<<_P
Dodaj katalog
_P;

$text['Add Page Bookmarklet'] = <<<_P
Dodaj Bookmarklet
_P;

$text['Add User to Group'] = <<<_P
Dodaj użytkownika do grupy
_P;

$text['All Bookmarks'] = <<<_P
Pokaż wszystkie linki
_P;

$text['Approve All Users'] = <<<_P
Zatwierdź wszystkich użytkowników
_P;

$text['Approve User'] = <<<_P
Zatwierdź użytkownika
_P;

$text['Approve Users'] = <<<_P
Zatwierdź użytkowników
_P;

$text['Bookmark News'] = <<<_P
Pokaż najnowsze linki
_P;

$text['Browse Folder'] = <<<_P
Przeglądaj katalog
_P;

$text['Contact Admin'] = <<<_P
Kontakt z Adminem
_P;

$text['Copy Bookmark'] = <<<_P
Nazwa linka źródłowego
_P;

$text['Copy'] = <<<_P
Kopiuj
_P;

$text['Create Group'] = <<<_P
Utwórz grupę
_P;

$text['Create Tree'] = <<<_P
Utwórz drzewo
_P;

$text['Custom Order'] = <<<_P
Kolejność niestandardowa
_P;

$text['Default User Settings'] = <<<_P
Ustawienia domyślne
_P;

$text['Delete Account'] = <<<_P
Usuń konto
_P;

$text['Delete Bookmark'] = <<<_P
Usuń link
_P;

$text['Delete Folder'] = <<<_P
Usuń katalog
_P;

$text['Delete Group'] = <<<_P
Usuń grupę
_P;

$text['Delete Tree'] = <<<_P
Usuń drzewo
_P;

$text['Delete User'] = <<<_P
Usuń użytkownika
_P;

$text['Display Topic'] = <<<_P
Wyświetl temat
_P;

$text['Download Bookmarks'] = <<<_P
Ściągnij zakładki
_P;

$text['Email Bookmark'] = <<<_P
Wyślij link
_P;

$text['Export Bookmarks'] = <<<_P
Eksport zakładek
_P;

$text['Export Description'] = <<<_P
Eksport opisów
_P;

$text['Favicon Management'] = <<<_P
Ustaw ikonę zakładki
_P;

$text['Filter Group RegExp'] = <<<_P
Filtruj grupy RegExp
_P;

$text['Filter Groups'] = <<<_P
Filtruj grupy
_P;

$text['Filter User RegExp'] = <<<_P
Filtruj użytkowników RegExp
_P;

$text['Filter Users'] = <<<_P
Filtruj użytkowników
_P;

$text['Folder Properties'] = <<<_P
Właściwości katalogu
_P;

$text['Folder Sharing'] = <<<_P
Udostępnianie folderu
_P;

$text['Get Bookmark Information'] = <<<_P
Pobierz informacje o linku
_P;

$text['Group Properties'] = <<<_P
Właściwości grupy
_P;

$text['Help'] = <<<_P
Pomoc
_P;

$text['Hide Folder'] = <<<_P
Ukryj katalog
_P;

$text['Import Bookmarks'] = <<<_P
Import zakładek
_P;

$text['Import Description'] = <<<_P
Import opisów
_P;

$text['Inactive Users'] = <<<_P
Nieaktywni użytkownicy
_P;

$text['Invite Members'] = <<<_P
Zaproś członków
_P;

$text['Log In'] = <<<_P
Logowanie
_P;

$text['Log Out'] = <<<_P
Wylogowanie
_P;

$text['Maintain Groups'] = <<<_P
Zarządzaj grupami
_P;

$text['Maintain Group'] = <<<_P
Zarządzaj grupą
_P;

$text['Maintain Trees'] = <<<_P
Zarządzaj drzewami
_P;

$text['Maintain Users'] = <<<_P
Zarządzaj użytkownikami
_P;

$text['Mark as Default'] = <<<_P
Zaznacz jako domyślny
_P;

$text['Modify User Settings'] = <<<_P
Modyfikuj ustawienia użytkownika
_P;

$text['Modify User'] = <<<_P
Modyfikuj użytkownika
_P;

$text['Most Active Users'] = <<<_P
Najbardziej aktywni użytkownicy
_P;

$text['Open Integrator'] = <<<_P
Otwórz Integrator
_P;

$text['Order of Trees'] = <<<_P
Kolejność drzew
_P;

$text['Paste'] = <<<_P
Wklej
_P;

$text['Pending Unverified Users'] = <<<_P
Oczekujący niezweryfikowani użytkownicy
_P;

$text['Pending Users'] = <<<_P
Oczekujący użytkownicy
_P;

$text['Pending Verified Users'] = <<<_P
Oczekujący zweryfikowani użytkownicy
_P;

$text['Personal Data'] = <<<_P
Dane osobiste
_P;

$text['Properties'] = <<<_P
Właściwości
_P;

$text['Purge Cache'] = <<<_P
Wyczyść cache
_P;

$text['Purge Folder'] = <<<_P
Wyczyść katalog
_P;

$text['Reject All Users'] = <<<_P
Odrzuć wszystkich użytkowników
_P;

$text['Reject Membership'] = <<<_P
Odrzuć członkowstwo
_P;

$text['Reject User'] = <<<_P
Odrzuć użytkownika
_P;

$text['Reject Users'] = <<<_P
Odrzuć użytkowników
_P;

$text['Reject'] = <<<_P
Odrzuć
_P;

$text['Remove User from Group'] = <<<_P
Usuń użytkownika z grupy
_P;

$text['Reset Password'] = <<<_P
Reset hasła
_P;

$text['Send Message to All'] = <<<_P
Wyślij wiadomość do wszystkich
_P;

$text['Send Message to Groups'] = <<<_P
Wyślij wiadomość do grup
_P;

$text['Send Message to Members'] = <<<_P
Wyślij wiadomość do członków
_P;

$text['Send Message to User'] = <<<_P
Wyślij wiadomość do użytkownika
_P;

$text['Session Settings'] = <<<_P
Ustawienia sesji
_P;

$text['Set Up'] = <<<_P
Ustaw
_P;

$text['Show All Icons'] = <<<_P
Pokaż wszystkie ikony
_P;

$text['Show Feed URL'] = <<<_P
Pokaż adres zakładki
_P;

$text['Sign Up'] = <<<_P
Zapisz się
_P;

$text['SiteBar Settings'] = <<<_P
Ustawienia SiteBar
_P;

$text['Undelete'] = <<<_P
Przywróć
_P;

$text['Unhide Subfolders'] = <<<_P
Odkryj podkatalogi
_P;

$text['Unhide Trees'] = <<<_P
Odkryj drzewa
_P;

$text['User Settings'] = <<<_P
Konfiguracja
_P;

$text['Validate Bookmarks'] = <<<_P
Sprawdź linki
_P;

$text['Validation'] = <<<_P
Sprawdzenie
_P;

$text['Verify Email'] = <<<_P
Weryfikacja emaila
_P;

$text['Admin Password'] = <<<_P
Hasło admina
_P;

$text['Account Approved'] = <<<_P
Konto potwierdzone
_P;

$text['Activity Period'] = <<<_P
Okres aktywności
_P;

$text['Add SiteBar Commands'] = <<<_P
Dodaj polecenia SiteBar
_P;

$text['Allow Anonymous Contact'] = <<<_P
Zezwalaj na kontakt anoninowy
_P;

$text['Allow Anonymous Exports'] = <<<_P
Zezwalaj na eksport anonimowy
_P;

$text['Allow Custom Web Search Engine'] = <<<_P
Zezwalaj na zmianę wyszukiwarki internetowej
_P;

$text['Allow Info Mail'] = <<<_P
Zezwalaj na email informacyjny
_P;

$text['Allow Sign Up'] = <<<_P
Zezwalaj na zapisanie
_P;

$text['Authentication Method'] = <<<_P
Metoda autoryzacji
_P;

$text['Auto Retrieve Favicon'] = <<<_P
Automatyczne pobranie ikon zakładek
_P;

$text['Base URL'] = <<<_P
Bazowy URL
_P;

$text['Bookmark Count'] = <<<_P
Liczba zakładek
_P;

$text['Bookmark File'] = <<<_P
Plik z zakładkami
_P;

$text['Bookmark Name'] = <<<_P
Nazwa linka
_P;

$text['Cached: '] = <<<_P
W cache
_P;

$text['Codepage'] = <<<_P
Strona kodowa
_P;

$text['Comment'] = <<<_P
Komentarz
_P;

$text['Confirm Invited Members'] = <<<_P
Potwierdź zaproszonych członków
_P;

$text['Content Only'] = <<<_P
Tylko zawartość
_P;

$text['Copyright'] = <<<_P
Prawa autorskie
_P;

$text['Create New Sub Folder'] = <<<_P
Utwórz nowy podkatalog
_P;

$text['Create User'] = <<<_P
Utwórz użytkownika
_P;

$text['Dead Bookmark'] = <<<_P
Nieaktualny link
_P;

$text['Decode Using'] = <<<_P
Dekoduj używając
_P;

$text['Default (%s)'] = <<<_P
Domyślny (%s)
_P;

$text['Default Search In'] = <<<_P
Domyślnie szukaj w
_P;

$text['Default Search Tool'] = <<<_P
Domyślne narzędzie wyszukiwania
_P;

$text['Default URL'] = <<<_P
Domyślny URL
_P;

$text['Delete Content Only'] = <<<_P
Usuń tylko zawartość
_P;

$text['Delete Invalid Favicons'] = <<<_P
Usuń błędne ikony zakładek
_P;

$text['Demo Account'] = <<<_P
Konto demonstracyjne
_P;

$text['Description File'] = <<<_P
Plik z opisem
_P;

$text['Description Import/Export'] = <<<_P
Import/Eksport opisów
_P;

$text['Description'] = <<<_P
Opis
_P;

$text['Discover Missing Favicons'] = <<<_P
Odkryj brakujące ikony zakładek
_P;

$text['E-mail Verified'] = <<<_P
email potwierdzony
_P;

$text['E-mail'] = <<<_P
email
_P;

$text['Encode Using'] = <<<_P
Zakoduj używając
_P;

$text['Exclude From Validation'] = <<<_P
Wyłącz ze sprawdzania
_P;

$text['Export Bookmarks Settings'] = <<<_P
Eksportuj ustawienia zakładek
_P;

$text['Favicon'] = <<<_P
Ikona zakładki
_P;

$text['Feedback or Other Comment'] = <<<_P
Sugestie lub komentarze
_P;

$text['Flatten the Hierarchy'] = <<<_P
Spłasz hierarchię
_P;

$text['Folder Name'] = <<<_P
Nazwa katalogu
_P;

$text['From'] = <<<_P
Od
_P;

$text['Group Name'] = <<<_P
Nazwa grupy
_P;

$text['Help Topic'] = <<<_P
Temat pomocy
_P;

$text['Hide XSLT Features'] = <<<_P
Ukryj funkcjonalności XSLT
_P;

$text['Ignore Private Bookmarks'] = <<<_P
Ignoruj prywatne linki
_P;

$text['Ignore Recently Tested'] = <<<_P
Ignoruj ostatnio testowane
_P;

$text['Include Subfolders'] = <<<_P
Załącz podkatalogi
_P;

$text['Integrator URL Selector'] = <<<_P
Wybierz URL Integratora
_P;

$text['Integrator URL'] = <<<_P
URL Integratora
_P;

$text['Keep Current URL'] = <<<_P
Zatrzymaj obecny URL
_P;

$text['Language'] = <<<_P
Język
_P;

$text['Limit Description Length'] = <<<_P
Ogranicz długość opisu
_P;

$text['Limit Number of Bookmarks'] = <<<_P
Ogranicz ilość linków
_P;

$text['Link Description Length'] = <<<_P
Długość opisu linka
_P;

$text['Link'] = <<<_P
Link
_P;

$text['Load Private Bookmarks Over SSL Only'] = <<<_P
Załaduj prywatne linki jedynie przez SSL
_P;

$text['Local Installation'] = <<<_P
Instalacja lokalna
_P;

$text['Managing Editor'] = <<<_P
Kierownik redakcji
_P;

$text['Maximal Icon Age'] = <<<_P
Maksymalny wiek ikon
_P;

$text['Maximal Icon Size'] = <<<_P
Maksymalny rozmiar ikon
_P;

$text['Maximal Icons Total'] = <<<_P
Maksymalna całkowita ilość ikon
_P;

$text['Maximum Session Time (sec)'] = <<<_P
Maksymalny czas sesji (sekundy)
_P;

$text['Mode'] = <<<_P
Tryb
_P;

$text['Moderator'] = <<<_P
Moderator
_P;

$text['New Tree Owner'] = <<<_P
Właściciel nowego drzewa
_P;

$text['Old Password'] = <<<_P
Stare hasło
_P;

$text['Only to Verified Emails'] = <<<_P
Tylko do potwierdzonych emaili
_P;

$text['Original: '] = <<<_P
Oryginał
_P;

$text['Owner'] = <<<_P
Właściciel
_P;

$text['Parent Folder'] = <<<_P
Katalog nadrzędny
_P;

$text['Password (visible to others)'] = <<<_P
Hasło (widoczne dla innych)
_P;

$text['Password'] = <<<_P
Hasło
_P;

$text['Paste Mode'] = <<<_P
Tryb wklejania
_P;

$text['Private'] = <<<_P
Prywatne
_P;

$text['Real Name'] = <<<_P
Prawdziwe imię
_P;

$text['Recent Time Expressed in Seconds'] = <<<_P
Ostatni czas wyrażony w sekundach
_P;

$text['Remember Me'] = <<<_P
Zapamiętaj mnie
_P;

$text['Remember as Default'] = <<<_P
Zapamiętaj jako domyślny
_P;

$text['Rename Duplicate Bookmarks'] = <<<_P
Zmień nazwy powtarzających się linków
_P;

$text['Repeat Admin Password'] = <<<_P
Powtórz hasło admina
_P;

$text['Repeat Password'] = <<<_P
Powtórz hasło
_P;

$text['Respect Allow Info Mail'] = <<<_P
Respektuj zezwolenie na email informacyjny
_P;

$text['Select Group'] = <<<_P
Wybierz grupę
_P;

$text['Select Input Format'] = <<<_P
Wybierz format wejściowy
_P;

$text['Select Output Format'] = <<<_P
Wybierz format wyjściowy
_P;

$text['Select Tree'] = <<<_P
Wybierz drzewo
_P;

$text['Select User'] = <<<_P
Wybierz użytkownika
_P;

$text['Sender E-mail'] = <<<_P
Email wysyłającego
_P;

$text['Show Favicons'] = <<<_P
Użyj ikon zakładek
_P;

$text['Show Logo'] = <<<_P
Pokaż logo
_P;

$text['Show Menu Icon'] = <<<_P
Pokaż ikonę menu
_P;

$text['Show Statistics'] = <<<_P
Pokaż statystyki
_P;

$text['Show Web Search Engine Results Inline'] = <<<_P
Pokaż rezultaty wyszukiwania wewnątrz
_P;

$text['Skin'] = <<<_P
Skórka
_P;

$text['Skip Execution Messages'] = <<<_P
Pomiń komunikaty wykonywania
_P;

$text['Source Folder Name'] = <<<_P
Nazwa katalogu źródłowego
_P;

$text['Target Folder Name'] = <<<_P
Nazwa katalogu docelowego
_P;

$text['Target'] = <<<_P
Cel
_P;

$text['Title Format for Selected Folder'] = <<<_P
Format tytułu dla wybranego katalogu
_P;

$text['Title for Root'] = <<<_P
Tytuł dla Root
_P;

$text['To'] = <<<_P
Do
_P;

$text['Topic'] = <<<_P
Temat
_P;

$text['Tree Name'] = <<<_P
Nazwa drzewa
_P;

$text['Tree Owner'] = <<<_P
Właściciel drzewa
_P;

$text['URL'] = <<<_P
URL
_P;

$text['Use Compression'] = <<<_P
Użyj kompresji
_P;

$text['Use Conversion Engine'] = <<<_P
Użyj silnika konwersji
_P;

$text['Use E-mail Features'] = <<<_P
Użyj funkcji email
_P;

$text['Use Folder Hiding'] = <<<_P
Użyj ukrywania katalogów
_P;

$text['Use Hit Counter'] = <<<_P
Użyj licznika odwiedzin
_P;

$text['Use Outbound Connection'] = <<<_P
Użyj połączeń Outbound
_P;

$text['Use SiteBar Tooltips'] = <<<_P
Użyj porad SiteBar 
_P;

$text['Use Trash'] = <<<_P
Użyj Śmietnika
_P;

$text['Use Web Search Engine'] = <<<_P
Użyj wyszukiwarek internetowych
_P;

$text['Use the Favicon Cache'] = <<<_P
Użyj cache ikon zakładek
_P;

$text['Users Can Create Groups'] = <<<_P
Użytkownicy mogą tworzyć grupy
_P;

$text['Users Can Create Trees'] = <<<_P
Użytkownicy mogą tworzyć drzewa
_P;

$text['Users Can Delete Trees'] = <<<_P
Użytkownicy mogą usuwać drzewa
_P;

$text['Users Must Be Approved'] = <<<_P
Konieczne potwierdzenie użytkowników
_P;

$text['Users Must Verify E-mail'] = <<<_P
Konieczna weryfikacja emaili użytkowników
_P;

$text['Verify E-mail'] = <<<_P
Weryfikacja emaila
_P;

$text['Web Search Engine Icon'] = <<<_P
Ikona strony wyszukiwania w sieci
_P;

$text['Web Search Engine URL'] = <<<_P
Adres strony wyszukiwania w sieci
_P;

$text['Webmaster Email'] = <<<_P
Email Webmastera
_P;

$text['Your E-mail'] = <<<_P
Twój email
_P;

$text['Open in New Window'] = <<<_P
Otwórz w nowym oknie
_P;

$text['Open as Plain Text'] = <<<_P
Otwórz jako czysty tekst
_P;

$text['Until I close browser'] = <<<_P
Do czasu zamknięcia przeglądarki
_P;

$text['12 hours'] = <<<_P
12 godzin
_P;

$text['6 days'] = <<<_P
6 dni
_P;

$text['1 month'] = <<<_P
1 miesiąc
_P;

$text['Maximum session time'] = <<<_P
Maksymalny czas sesji
_P;

$text['Order of Folders v. Bookmarks'] = <<<_P
Kolejność katalogów i linków
_P;

$text['Bookmarks First'] = <<<_P
Linki jako pierwsze
_P;

$text['Folders First'] = <<<_P
Katalogi jako pierwsze
_P;

$text['Default Bookmark Sort Mode'] = <<<_P
Domyślny tryb sortowania katalogów
_P;

$text['Sort Mode'] = <<<_P
Tryb sortowania
_P;

$text['User Default'] = <<<_P
Domyślne ustawienia użytkownika
_P;

$text['Alphabetically'] = <<<_P
Alfabetycznie
_P;

$text['Most Popular'] = <<<_P
Najbardziej popularne
_P;

$text['Recently Added'] = <<<_P
Ostatnio dodane
_P;

$text['Recently Modified'] = <<<_P
Ostatnio zmodyfikowane
_P;

$text['Recently Visited'] = <<<_P
Ostatnio odwiedzane
_P;

$text['Waiting for Visit'] = <<<_P
Oczekiwanie
_P;

$text['Message'] = <<<_P
Wiadomość
_P;

$text['SiteBar Server'] = <<<_P
Serwer SiteBar 
_P;

$text['%s - ok.'] = <<<_P
%s - ok.
_P;

$text['%s - error!'] = <<<_P
%s - błąd!
_P;

$text['SiteBar: Account Request Approved'] = <<<_P
SiteBar: Prośba o konto zaakceptowana
_P;

$text['SiteBar: Account Request Rejected'] = <<<_P
SiteBar: Prośba o konto odrzucona
_P;

$text['SiteBar: Contact Admin'] = <<<_P
SiteBar: Kontakt z Adminem
_P;

$text['SiteBar: Email Verification'] = <<<_P
SiteBar: Potwierdzenie Email
_P;

$text['SiteBar: New SiteBar User'] = <<<_P
SiteBar: Nowy użytkownik SiteBar
_P;

$text['SiteBar: New SiteBar User Verified E-mail'] = <<<_P
SiteBar: Zweryfikowany email nowego użytkownika
_P;

$text['SiteBar: Reset Password'] = <<<_P
SiteBar: Reset hasła
_P;

$text['SiteBar: Web site'] = <<<_P
SiteBar: Strona web
_P;

$text['Group'] = <<<_P
Grupa
_P;

$text['R'] = <<<_P
R
_P;

$text['A'] = <<<_P
A
_P;

$text['M'] = <<<_P
M
_P;

$text['D'] = <<<_P
D
_P;

$text['Ask'] = <<<_P
Pytanie
_P;

$text['Move or Copy'] = <<<_P
Przenieś lub kopiuj
_P;

$text['Copy (Keep Source)'] = <<<_P
Kopiuj (zachowaj żródło)
_P;

$text['Move (Delete Source)'] = <<<_P
Przenieś (usuń źródło)
_P;

$text['Auto detection'] = <<<_P
Automatyczne wykrywanie
_P;

$text['Netscape/Mozilla/MS IE'] = <<<_P
Netscape/Mozilla/MS IE
_P;

$text['Opera Hotlist version 2.0'] = <<<_P
Opera Hotlist version 2.0
_P;

$text['Admins'] = <<<_P
Admini
_P;

$text['SiteBar Bookmarks'] = <<<_P
Zakładki SiteBar
_P;

$text['Bookmarks from SiteBar installation at'] = <<<_P
Zakłaski z instalacji SiteBar na
_P;

$text['Up'] = <<<_P
Góra
_P;

$text['Search in SiteBar Bookmarks'] = <<<_P
Szukaj w zakładkach SiteBar 
_P;

$text['All'] = <<<_P
Wszystko
_P;

$text['Name'] = <<<_P
Nazwa
_P;

$text['Access denied!'] = <<<_P
Dostęp zabroniony
_P;

$text['All links recently validated!'] = <<<_P
Wszystkie linki niedawno sprawdzone!
_P;

$text['Cannot connect to http://%s:%s!'] = <<<_P
Nie można nawiązać połączenia z adresem http://%s:%s!
_P;

$text['Cannot download %s using https, will try http!'] = <<<_P
Nie można ściągnąć %s używając https, próba poprzez http!
_P;

$text['Cannot export empty description!'] = <<<_P
Nie można wyeksportować pustego opisu
_P;

$text['Cannot handle %s protocol!'] = <<<_P
Nie można obsłużyć protokołu %s!
_P;

$text['Cannot move folder to its subfolder!'] = <<<_P
Nie można przenieść katalogu do jego podkatalogu!
_P;

$text['Connection timed out!'] = <<<_P
Przekroczony czas oczekiwania na połączenie!
_P;

$text['Database corrupted - missing anonymous account!'] = <<<_P
Uszkodzona baza danych - brak anonimowego konta!
_P;

$text['Description too long for inplace editing, please use export feature!'] = <<<_P
Opis zbyt długi do edycji w tym samym miejscu, użyj funkcji eksportu!
_P;

$text['Duplicate folder name "%s"!'] = <<<_P
Powtórzona nazwa katalogu "%s"!
_P;

$text['Duplicate name "%s" in the target folder!'] = <<<_P
Powtórzona nazwa "%s" w katalogu docelowym!
_P;

$text['Duplicate name "%s"!'] = <<<_P
Powtórzona nazwa "%s"!
_P;

$text['Favicon <img src="%s"> found at url %s.'] = <<<_P
Ikona zakładki <img src="%s"> znaleziona pod adresem %s.
_P;

$text['Favicon not found! Trying default favicon location %s.'] = <<<_P
Ikona skrótu nie znaleziona! Próba użycia dmyślnej lokalizacji ikony zakładki %s.
_P;

$text['Favicon not found!'] = <<<_P
Ikona zakładki nie znaleziona!
_P;

$text['Folder has already been deleted!'] = <<<_P
Katalog został usunięty!
_P;

$text['Folder with id %s does not exist!'] = <<<_P
Katalog o id %s  nie istnieje!
_P;

$text['Group name "%s" is already used!'] = <<<_P
Grupa o nazwie "%s" już istnieje
_P;

$text['HEAD unsupported, trying GET!'] = <<<_P
HEAD nie obsługiwany, próba GET!
_P;

$text['Head of the HTML page was not found!'] = <<<_P
Nagłówek strony HTML nie został znaleziony!
_P;

$text['Host or port undefined!'] = <<<_P
Host lub port niezdefiniowany!
_P;

$text['Incomplete HTTP response!'] = <<<_P
Niekompletna odpowiedź HTTP!
_P;

$text['Invalid filename or other upload related problem: %s!'] = <<<_P
Nieprawidłowa nazwa lub inny problem związany  z ładowaniem %s!
_P;

$text['Invalid regular expresssion!'] = <<<_P
Nieprawidłowe wyrażenie regularne!
_P;

$text['Link already is in the target folder!'] = <<<_P
Link już jest w katalogu docelowym!
_P;

$text['Link has already been deleted!'] = <<<_P
Link został już usunięty!
_P;

$text['Missing command!'] = <<<_P
Brak polecenia!
_P;

$text['No action taken!'] = <<<_P
Nie podjęto żadnej akcji!
_P;

$text['No links in the folder!'] = <<<_P
Brak linków w katalogu!
_P;

$text['No more than %s redirections allowed!'] = <<<_P
Nie więcej niż %s dozwolonych przekierowań!
_P;

$text['No user was selected!'] = <<<_P
Nie wybrano użytkownika!
_P;

$text['No users are pending!'] = <<<_P
Brak oczekujących użytkowników!
_P;

$text['Node number %s has ACL record but does not exist!'] = <<<_P
Node numer %s posiada wpis ACL ale nie istnieje!
_P;

$text['Old password is invalid!'] = <<<_P
Stare hasło jes nieprawidłowe!
_P;

$text['Page not found!'] = <<<_P
Strona nie znaleziona!
_P;

$text['Page redirected to %s!'] = <<<_P
Strona przekierowana do %s!
_P;

$text['Please click on the "%s" command to finish the verification procedure.'] = <<<_P
Kliknij proszę na  polecenie "%s" i zakończ procedurę weryfikacji.
_P;

$text['Purge folder to remove it permanently!'] = <<<_P
Usuń katalog nieodwracalnie!
_P;

$text['The description length exceeds maximum length of %s bytes!'] = <<<_P
Długość opisu przekroczyła maksymalną %s bajtów!
_P;

$text['The e-mail %s does not look correctly!'] = <<<_P
Email %s wygląda na niepoprawny!
_P;

$text['The password was not repeated correctly!'] = <<<_P
Hasło nie zostało powtórzone poprawnie!
_P;

$text['There are no hidden trees!'] = <<<_P
Brak ukrytych drzew!
_P;

$text['There is no content to be deleted!'] = <<<_P
Brak zawartości do usunięcia!
_P;

$text['There is nothing to be undeleted!'] = <<<_P
Nie ma nic do odzyskania!
_P;

$text['This SiteBar server requires your email address to be verified in order to use some functions.'] = <<<_P
Ten serwer SiteBar wymaga weryfikacji Twojego adresu email aby móc używać część funkcji.
_P;

$text['Tree has already been deleted!'] = <<<_P
Drzewo zostało już usunięte!
_P;

$text['Unknown command!'] = <<<_P
Nieznane polecenie!
_P;

$text['Unknown username or wrong password!'] = <<<_P
Zła nazwa użytkownika lub hasło!
_P;

$text['Use "%s" command to delete own account!'] = <<<_P
Użycie polecenia "%s" spowoduje usunięcie własnego konta!
_P;

$text['User with email "%s" does not exist!'] = <<<_P
Użytkownik z emailem "%s" nie istnieje!
_P;

$text['User with uid %d has already been rejected!'] = <<<_P
Użytkownik o uid %d został już odrzucony!
_P;

$text['Verification e-mail has been sent to your e-mail address!'] = <<<_P
Email sprawdzający został wysłany na Twój adres email!
_P;

$text['Wrong favicon type/format "%s"!'] = <<<_P
Zły typ/format ikony zakładki "%s"!
_P;

$text['Successful execution!'] = <<<_P
Wykonanie udane!
_P;

$text['Close'] = <<<_P
Zamknij
_P;

$text['Imported %s link(s) into %s folder(s) from the bookmark file.'] = <<<_P
Zaimportowano %s link(i) do %s katalogu(ów) z pliku zakładek.
_P;

$text['Invalid or expired token received! All pending tokens were deleted.'] = <<<_P
Nieprawidłowy lub wygasły token! Wszystkie oczekujące tokeny zostały usunięte.
_P;

$text['Link has been added.<p>You must reload your SiteBar in order to see added link!'] = <<<_P
Link został dodany.<p>Musisz odświeżyć SiteBar aby zobaczyć dodany link!
_P;

$text['Password has been changed!'] = <<<_P
Hasło zostało zmienione
_P;

$text['Reload SiteBar'] = <<<_P
Odśwież SiteBar
_P;

$text['Reset'] = <<<_P
Reset
_P;

$text['Return'] = <<<_P
Powrót
_P;

$text['Submit'] = <<<_P
Zatwierdź
_P;

$text['Your account will be activated as soon as one of the administrators approves it.'] = <<<_P
Twoje konto zostanie aktywowane jak tylko któryś z administratorów je zaakceptuje.
_P;

$text['Cannot open file!'] = <<<_P
Nie można otworzyć pliku
_P;

$text['Unknown bookmark file format!'] = <<<_P
Nieznany format pliku
_P;

$text['File is empty!'] = <<<_P
Plik jest pusty
_P;

$text['Character set overriden from document to %s!'] = <<<_P
Zestaw znaków w dokumencie zastąpiony  na %s
_P;

$text['There is no conversion engine available to convert from %s character set!'] = <<<_P
Brak możliwości konwesji zestawu znaków z %s
_P;

$text['Powered by %s ver. %s'] = <<<_P
Powered by %s ver. %s
_P;

$text['Skin created by'] = <<<_P
Skórka stworzona przez
_P;

$text['Skin designed by'] = <<<_P
Skórka zaprojektowana przez
_P;
